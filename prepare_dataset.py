...
Пайплайн (ни один файл не проходит без проверки):
...
def clean_text(text):
    """Убираем control/format/private-use/surrogate символы - источник
    раздутого алфавита и мусора из битых кодировок/веб-скрейпа."""
    cleaned = "".join(
        ch for ch in text
        if unicodedata.category(ch) not in _JUNK_UNICODE_CATEGORIES or ch in ("\n", "\t")
    )
    # схлопываем множественные пробелы (но не переносы строк - важны для структуры)
...
def passes_quality(text, min_chars=200, min_alpha_ratio=0.6, max_repeated_line_ratio=0.3):
    ...
    Отсеиваем:
      ...
      - "нечитаемый" текст: мало букв относительно всех непробельных символов
        (спам из спецсимволов/чисел/мусора)
      - шаблонный спам: если одна и та же строка повторяется слишком часто
        (типичный признак меню/футера сайта, накопленного при скрейпе)
    """
...
def normalized_hash(text):
    """Hash по тексту с нормализованными пробелами и регистром - ловит
    точные и почти-точные (разница только в пробелах/регистре) дубли."""
...
def compute_simhash(text, hashbits=64, shingle_size=5):
    """
    SimHash: документы с похожим содержанием получают близкие (по Хэммингу)
    64-битные отпечатки. Считаем по shingles (скользящие окна из N слов).
    """
    words = text.split()
...
def find_near_duplicates(simhashes, hashbits=64, n_bands=4, max_distance=3):
    """
    LSH banding: делим 64-битный отпечаток на n_bands кусков. Документы,
    совпадающие хотя бы в одном "банде", попадают в общий бакет-кандидат.
    Внутри маленьких бакетов сравниваем честно по Хэммингу (быстро, т.к.
    бакеты небольшие) - так что не O(n^2) по всему корпусу, а только
    O(n^2) внутри каждого маленького бакета кандидатов.

    ВАЖНО: SimHash надёжно ловит почти-дубли на ДЛИННЫХ документах (где
    изменённая часть - малая доля от общего текста). На очень коротких
    текстах (пара предложений) чувствительность ниже - одно изменённое
    слово может непропорционально сильно сдвинуть отпечаток. Для корпуса
    из полноразмерных файлов (статьи, диалоги, игровые тексты) это не
    проблема; если ваши файлы очень короткие - имеет смысл поднять
    max_distance или полагаться в основном на точную дедупликацию (шаг 4).

    Возвращает set индексов файлов, которые надо ВЫКИНУТЬ (оставляем
    первое вхождение из каждой группы дублей).
    """
        ...
        for i in range(len(idxs)):
            for j in range(i + 1, len(idxs)):
                ...
                if hamming_distance(simhashes[a], simhashes[b]) <= max_distance:
                    to_drop.add(max(a, b))  # оставляем первое (меньший индекс)
...
def clean_and_dedup_files(files, min_chars, min_alpha_ratio, max_repeated_line_ratio,
    ...
    """
    Полный проход [2]-[5]: читаем каждый файл один раз, чистим, фильтруем
    по качеству, считаем хэши для точной и почти-дубликатной дедупликации.
    Текст в память целиком не копим - только легковесные хэши + пути.
    """
    print(f"[2-4/6] Очистка + фильтр качества + точная дедупликация "
          f"({len(files)} файлов)...")

    survivors = []          # пути файлов, прошедших очистку+качество+точный дедуп
    simhashes = []          # simhash для каждого survivor (для этапа 5)
    seen_hashes = set()
    stats = Counter()
...
def tokenize_and_chunk(file_paths, tokenizer, context_length):
    """
    ВАЖНО: используем tokenizer.encode(text) уровня HF (не tokenizer._tokenizer
    напрямую) с add_special_tokens=True - post_processor уже добавляет
    <|bos|>...<|eos|> ОДИН раз через TemplateProcessing. Раньше здесь был
    баг: brute-force _tokenizer.encode() уже применяет post_processor,
    а следом ids вручную ЕЩЁ РАЗ оборачивались в bos/eos - двойные
    спецтокены на каждом чанке.
    """
    token_buffer = []
    chunks = []
    ...
    for path in file_paths:
        ...
        # add_special_tokens=True -> post_processor добавит bos/eos ОДИН раз
        ...
        while len(token_buffer) - cursor >= context_length:
            chunk = token_buffer[cursor:cursor + context_length]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--near-dup-distance", type=int, default=3,
                         help="Порог Хэмминга для SimHash (меньше = строже, только очень похожие)")
